...
function deriveParamsFromQuery(searchParams: ReadonlyURLSearchParams | null): Params {
...
function parseScenarioPayload(raw: string): ScenarioPayload | null {
  try {
    ...
    // If the value is base64 encoded, attempt to decode and parse again.
    if (typeof atob === "function") {
      try {
        return validateScenarioPayload(JSON.parse(atob(raw)));
      } catch {
        return null;
      }
    }
...
function generateSmartDefaults(): Params {
  const now = new Date();
