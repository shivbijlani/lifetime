...
When exporting for GitHub Pages, set `NEXT_PUBLIC_BASE_PATH=/lifetime` (or your repository name) before running `npm run export`. If you use a custom domain, set `NEXT_PUBLIC_BASE_PATH=/` or run `CUSTOM_DOMAIN=yourdomain.com npm run export` to emit root-relative assets.
